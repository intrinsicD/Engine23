    ...
    namespace SystemCameraInternal {
        ...
        void on_render_gui(const Events::Render<Gui> &event) {
            ...
            if (ImGui::Begin("Camera", &show_gui)) {
                ...
                if (ImGui::CollapsingHeader("View Parameters")) {
                    auto view_parameters = camera.get_view_parameters();
                    if (!edit) {
                        ImGui::Text("Position: %f %f %f", view_parameters.position.x, view_parameters.position.y,
                                    view_parameters.position.z);
                        ImGui::Text("Direction: %f %f %f", view_parameters.direction.x, view_parameters.direction.y,
                                    view_parameters.direction.z);
                        ImGui::Text("Up: %f %f %f", view_parameters.up.x, view_parameters.up.y, view_parameters.up.z);
                    } else {
                        static bool changed_view = false;
                        static Camera::ViewParameters view_parameters_new = view_parameters;
                        changed_view |= ImGui::InputFloat3("Position", &view_parameters_new.position.x);
                        changed_view |= ImGui::InputFloat3("Direction", &view_parameters_new.direction.x);
                        changed_view |= ImGui::InputFloat3("Up", &view_parameters_new.up.x);
                        if (changed_view) {
                            if (ImGui::Button("Apply##View")) {
                                changed_view = false;
                                camera.set_view_parameters(view_parameters_new);
                            }
                        }
                    }
                    if (ImGui::Button("Reset##View")) {
                        camera.set_view_parameters({glm::vec3(0.0f, 0.0f, 3.0f), glm::vec3(0.0f, 0.0f, -1.0f),
        ...
        void on_update_input(const Events::Update<Input> &event) {
            ...
            if (keyboard.keys[GLFW_KEY_W]) {
                //move forward
                view_parameters.position += view_parameters.direction * delta;
                changed = true;
            }
            ...
            if (keyboard.keys[GLFW_KEY_D]) {
                //move right
                view_parameters.position += right * delta;
                changed = true;
            }
            ...
            if (changed) {
                camera.set_view_parameters(view_parameters);
            }
        }
        ...
        void on_update_fps_controller(const Events::Update<Input::Mouse::Position> &event){
            ...
            auto view_params = fps.camera.get_view_parameters();
            view_params.direction = glm::normalize(front);
            fps.camera.set_view_parameters(view_params);
        }
